...
def build_model(bert_path, ofa_path,mode='hybrid'):
    ...
    if mode == 'hybrid' or mode == 'location':
        ...
        ofa_path = 'OFA-Sys/ofa-base'
        tokenizer_vqa = AutoTokenizer.from_pretrained(ofa_path)
        model_vqa = AutoModel.from_pretrained(ofa_path, use_cache=False)
        model_vqa.eval()
...
def masked_prompt(cls_names, model, tokenizer, mode='hybrid', topk=3):
    ...
    for cls_name in cls_names:
        ...
        for k, v in questions_dict.items():
            ...
            masked_index = tokenized_text.index('[MASK]') # 找到[MASK]的位置
            with torch.no_grad():
                predictions = model(tokens_tensor, segments_tensors)
            
            _, predicted_index = torch.topk(predictions[0][0][masked_index], topk)
            # predictions[0][0][masked_index]    torch.Size([30522])
