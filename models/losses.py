    ...
    def forward(self, outputs):
        ...
        if self.args.text_mode == 'sentence':
            ...
            for k, text_embed in text_embed_all.items():
                logits[:, list(text_embed_all.keys()).index(k) ] = (logit_scale * image_embed_all @ text_embed.t()).mean(dim=-1)
...
class CITEImage_ContGene_Loss(nn.Module):
    def __init__(self, args):
        super().__init__()
        self.labels = None
        self.last_local_batch_size = None
        ...
        self.image_gene_cont_type = args.image_gene_cont_type
    ...
    def forward(self, outputs):
        ...
        # Cross-entropy
        if self.args.text_mode == 'sentence':
            image_logits = (logit_scale * image_embed_all @ text_embed_all.t())
            omic_logits = (logit_scale * omic_embed_all @ text_embed_all.t())
        elif self.args.text_mode == 'description':
            image_logits =  torch.zeros((local_batch_size, len(text_embed_all))).cuda() # [B,3]
            omic_logits =  torch.zeros((local_batch_size, len(text_embed_all))).cuda() # [B,3]
            for k, text_embed in text_embed_all.items():
                image_logits[:, list(text_embed_all.keys()).index(k) ] = (logit_scale * image_embed_all @ text_embed.t()).mean(dim=-1)
                omic_logits[:, list(text_embed_all.keys()).index(k) ] = (logit_scale * omic_embed_all @ text_embed.t()).mean(dim=-1)

        image_text_cls_loss = self.args.w_image_text * F.cross_entropy(image_logits, cls_label)
        # loss = text_cls_loss
        omic_text_cls_loss =  self.args.w_omic_text * F.cross_entropy(omic_logits, cls_label)

        # loss = 
        # (F.cross_entropy(logits_per_omic_text, self.labels) + F.cross_entropy(logits_per_text_omic, self.labels) )/ 2 + \
        #         (F.cross_entropy(logits_per_omic_image, self.labels) + F.cross_entropy(logits_per_image_omic, self.labels)) / 2

        if local_batch_size != self.last_local_batch_size:
            self.cont_labels = local_batch_size * utils.get_rank() + torch.arange(
                local_batch_size, device=omic_embed.device
            )  # [B, ]
            self.last_local_batch_size = local_batch_size
        
        logits_per_omic_image = logit_scale * omic_embed @ image_embed_all.t() # [B,B]
        logits_per_image_omic = logit_scale * image_embed @ omic_embed_all.t() # 
        ...
        loss = image_text_cls_loss + omic_text_cls_loss + image_omic_cont_loss
        ...
        # compute accuracy
        with torch.no_grad():
            pred = torch.argmax(omic_logits, dim=-1)
            correct = pred.eq(cls_label).sum()
            omic_text_cls_acc = 100 * correct / local_batch_size

            pred = torch.argmax(logits_per_omic_image, dim=-1)
            correct = pred.eq(self.cont_labels).sum()
            image_omic_cont_acc = 100 * correct / local_batch_size

            pred = torch.argmax(image_logits, dim=-1)
            correct = pred.eq(cls_label).sum()
            image_text_cls_acc = 100 * correct / local_batch_size
        ...
        return {'loss': loss, 
        'image_text_cls_loss': image_text_cls_loss, 'omic_text_cls_loss': omic_text_cls_loss, 'image_omic_cont_loss': image_omic_cont_loss, 'image_text_cls_acc': image_text_cls_acc, 'omic_text_cls_acc': omic_text_cls_acc, 'image_omic_cont_acc': image_omic_cont_acc}
        
        
        # 'ulip_omic_image_matching_acc': omic_image_acc, 'ulip_omic_text_matching_acc': omic_text_acc, 'ulip_image_text_matching_acc': image_text_acc}

        # return {'loss': loss, 'image_text_matching_acc': image_text_acc}


class CITEImage_ContGene_Graph_Loss(nn.Module):
    def __init__(self, args):
        super().__init__()
        self.labels = None
        self.last_local_batch_size = None
    ...
    def forward(self, outputs):
        ...
        with torch.no_grad():
            ...
            pred = torch.argmax(outputs['graph_logits'], dim=-1)
            correct = pred.eq(cls_label).sum()
...
class VisClsLoss(nn.Module):
    ...
    def forward(self, outputs):
        ...
        # image_embed_all, text_embed_all = utils.all_gather_batch([image_embed, text_embed])
        # image_embed_all = utils.all_gather_batch([image_embed])
        ...
        # return {'loss': loss, 'ulip_loss': loss, 'ulip_omic_image_matching_acc': omic_image_acc, 'ulip_omic_text_matching_acc': omic_text_acc, 'ulip_image_text_matching_acc': image_text_acc}
        return {'loss': loss, 'image_text_cls_acc': image_text_cls_acc}